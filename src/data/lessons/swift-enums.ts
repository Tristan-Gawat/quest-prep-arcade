// Pre-written lessons for Swift Module: Enums

export const swiftEnumsLessons = [
  {
    title: "What is Enums?",
    definition: "Enums in Swift are first-class types with associated values, computed properties, methods, and pattern matching. Far more powerful than C-style enums, they model state machines and algebraic data types.",
    explanation: `Enums is a core concept in Swift that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Swift's approach to enums is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering enums, you unlock powerful patterns and idioms that are standard in professional Swift development.`,
    code: `// Enums in Swift - Basics
import Foundation
...
print("\(hero.name): \(hero.hp)/\(hero.maxHp)")`,
    breakdown: `\u2022 The basic declaration shows how to define and use enums in Swift.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Enums in Swift provides are first-class types with associated values, computed properties, methods, and pattern matching. Far more powerful than C-style enums, they model state machines and algebraic data types.. It's fundamental to writing correct, efficient Swift code."
  },
  {
    title: "How Enums works",
    definition: "Enums works by being value types with a fixed set of cases. Associated values store data per-case. Pattern matching (switch) enables exhaustive handling. Raw values provide fixed mappings to primitives.",
    explanation: `Under the hood, enums in Swift involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how enums works internally helps you predict behavior, debug issues, and write more efficient code.
...
enum GameState {
    ...
    summary: "Enums works through being value types with a fixed set of cases. Associated values store data per-case. Pattern matching (switch) enables exhaustive handling. Raw values provide fixed mappings to primitives.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Enums syntax & usage",
    definition: "Swift enums syntax includes enum Name { case variant }, associated values (case .variant(data)), raw values (: Int/String), switch exhaustive matching, if case pattern, and CaseIterable.",
    explanation: `Swift provides clear syntax for enums with several variations depending on your needs. The standard library builds extensively on these foundations.
...
case .failure(let error):
    ...
    summary: "Swift syntax for enums is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Enums",
    definition: "In real applications, enums models states (loading/success/error), navigation routes, settings options, game events, and API responses with different data shapes.",
    explanation: `Real-world Swift applications use enums for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Swift projects provide excellent examples of enums in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Enums - Practical game system
...
func handle(_ event: GameEvent) {
    ...
    summary: "Real applications demonstrate enums in game systems, data processing, and service design. The patterns are universal across Swift projects."
  },
  {
    title: "Enums best practices",
    definition: "Best practices for enums include using associated values over separate properties, making enums CaseIterable when appropriate, using raw values for serialization, and leveraging exhaustive switch for safety.",
    explanation: `Professional Swift code follows established conventions for enums that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when enums is used correctly as well-structured code is inherently more testable.
...
// DO: Use if-let or guard-let instead`,
    ...
    summary: "Best practices ensure code quality: using associated values over separate properties, making enums CaseIterable when appropriate, using raw values for serialization, and leveraging exhaustive switch for safety.. Following conventions makes code maintainable and professional."
  }
];
