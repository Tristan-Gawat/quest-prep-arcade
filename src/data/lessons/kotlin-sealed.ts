// Pre-written lessons for Kotlin Module: Sealed Classes

export const kotlinSealedLessons = [
  {
    title: "What is Sealed Classes?",
    definition: "Sealed Classes in Kotlin restrict class hierarchies to a fixed set of subclasses defined in the same file. Combined with when expressions, they enable exhaustive pattern matching on type hierarchies.",
    explanation: `Sealed Classes is a core concept in Kotlin that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Kotlin's approach to sealed classes is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering sealed classes, you unlock powerful patterns and idioms that are standard in professional Kotlin development.`,
    code: `// Sealed Classes in Kotlin - Basics
fun main() {
    println("Sealed Classes in Kotlin")
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use sealed classes in Kotlin.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Sealed Classes in Kotlin provides restrict class hierarchies to a fixed set of subclasses defined in the same file. Combined with when expressions, they enable exhaustive pattern matching on type hierarchies.. It's fundamental to writing correct, efficient Kotlin code."
  },
  {
    title: "How Sealed Classes works",
    definition: "Sealed Classes works by the compiler knowing ALL possible subclasses at compile time (they must be in the same package). When expressions can verify all cases are handled without a default branch.",
    explanation: `Under the hood, sealed classes in Kotlin involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how sealed classes works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `// Sealed Classes - How it works
...
val hp = 150.clamp(0, 100) // 100`,
    ...
    summary: "Sealed Classes works through the compiler knowing ALL possible subclasses at compile time (they must be in the same package). When expressions can verify all cases are handled without a default branch.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Sealed Classes syntax & usage",
    definition: "Kotlin sealed classes syntax includes sealed class/interface, subclass definitions, when without else (exhaustive), data class subclasses for carrying data, and object subclasses for singletons.",
    explanation: `Kotlin provides clear syntax for sealed classes with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Kotlin continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Sealed Classes - Syntax patterns
...
value class Gold(val amount: Int) {
    ...
    summary: "Kotlin syntax for sealed classes is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Sealed Classes",
    definition: "In real applications, sealed classes models UI states (Loading/Success/Error), navigation events, network responses, and any domain where the set of possible types is fixed and known.",
    explanation: `Real-world Kotlin applications use sealed classes for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Kotlin projects provide excellent examples of sealed classes in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Sealed Classes - Practical game example
...
fun CoroutineScope.gameLoop() = launch {
    ...
    summary: "Real applications demonstrate sealed classes in game systems, data processing, and service design. The patterns are universal across Kotlin projects."
  },
  {
    title: "Sealed Classes best practices",
    definition: "Best practices for sealed classes include using sealed classes for state machines, combining with data classes for variant data, using sealed interfaces for flexibility, and leveraging exhaustive when for safety.",
    explanation: `Professional Kotlin code follows established conventions for sealed classes that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when sealed classes is used correctly as well-structured code is inherently more testable.
...
Performance and safety are balanced through careful application of these principles. Knowing when to optimize and when readability matters more is a key skill.`,
    code: `// Sealed Classes - Best practices
...
inline fun <T> measureTime(block: () -> T): Pair<T, Long> {
    ...
    summary: "Best practices ensure code quality: using sealed classes for state machines, combining with data classes for variant data, using sealed interfaces for flexibility, and leveraging exhaustive when for safety.. Following conventions makes code maintainable and professional."
  }
];
