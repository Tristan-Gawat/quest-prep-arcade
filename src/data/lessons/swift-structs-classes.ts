// Pre-written lessons for Swift Module: Structs & Classes

export const swiftStructsClassesLessons = [
  {
    title: "What is Structs & Classes?",
    definition: "Structs & Classes in Swift are both custom types with properties and methods, but structs are VALUE types (copied) and classes are REFERENCE types (shared). Swift strongly prefers structs for most data types.",
    explanation: `Structs & Classes is a core concept in Swift that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Swift's approach to structs & classes is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering structs & classes, you unlock powerful patterns and idioms that are standard in professional Swift development.`,
    code: `// Structs & Classes in Swift - Basics
import Foundation
...
print("\(hero.name): \(hero.hp)/\(hero.maxHp)")`,
    breakdown: `\u2022 The basic declaration shows how to define and use structs & classes in Swift.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Structs & Classes in Swift provides are both custom types with properties and methods, but structs are VALUE types (copied) and classes are REFERENCE types (shared). Swift strongly prefers structs for most data types.. It's fundamental to writing correct, efficient Swift code."
  },
  {
    title: "How Structs & Classes works",
    definition: "Structs & Classes works by structs copying on assignment (value semantics) while classes share references. Copy-on-write optimization makes struct copies efficient until mutation. Classes support inheritance; structs don’t.",
    explanation: `Under the hood, structs & classes in Swift involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how structs & classes works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `// Structs & Classes - How it works
...
enum GameState {
    ...
    summary: "Structs & Classes works through structs copying on assignment (value semantics) while classes share references. Copy-on-write optimization makes struct copies efficient until mutation. Classes support inheritance; structs don’t.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Structs & Classes syntax & usage",
    definition: "Swift structs & classes syntax includes struct/class keyword, properties (stored, computed, lazy), methods (mutating for structs), initializers, deinit (classes only), and protocol conformance.",
    explanation: `Swift provides clear syntax for structs & classes with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Swift continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Structs & Classes - Syntax patterns
...
case .failure(let error):
    ...
    summary: "Swift syntax for structs & classes is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Structs & Classes",
    definition: "In real applications, structs & classes structs model coordinates, colors, configurations; classes model shared state, UI controllers, and reference-counted resources.",
    explanation: `Real-world Swift applications use structs & classes for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Swift projects provide excellent examples of structs & classes in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Structs & Classes - Practical game system
...
func handle(_ event: GameEvent) {
    ...
    summary: "Real applications demonstrate structs & classes in game systems, data processing, and service design. The patterns are universal across Swift projects."
  },
  {
    title: "Structs & Classes best practices",
    definition: "Best practices for structs & classes include defaulting to structs unless you need reference semantics, using classes for identity/inheritance, marking classes as final when not designed for subclassing, and leveraging value semantics for thread safety.",
    explanation: `Professional Swift code follows established conventions for structs & classes that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when structs & classes is used correctly as well-structured code is inherently more testable.
...
Performance and safety are balanced through careful application of these principles. Knowing when to optimize and when readability matters more is a key skill.`,
    code: `// Structs & Classes - Best practices
...
// DO: Use if-let or guard-let instead`,
    ...
    summary: "Best practices ensure code quality: defaulting to structs unless you need reference semantics, using classes for identity/inheritance, marking classes as final when not designed for subclassing, and leveraging value semantics for thread safety.. Following conventions makes code maintainable and professional."
  }
];
