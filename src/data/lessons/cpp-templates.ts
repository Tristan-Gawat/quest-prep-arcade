// Pre-written lessons for C++ Module: Templates

export const cppTemplatesLessons = [
  {
    title: "What is Templates?",
    definition: "Templates in C++ enable generic programming by parameterizing types and values at compile time, generating specialized code for each type used.",
    explanation: `Templates is a core concept in C++ that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
C++'s approach to templates is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering templates, you unlock powerful patterns and idioms that are standard in professional C++ development.`,
    code: `// Templates - Basic usage in C++
#include <iostream>
#include <memory>
#include <vector>
using namespace std;
...
int main() {
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use templates in C++.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Templates in C++ provides enable generic programming by parameterizing types and values at compile time, generating specialized code for each type used.. It's fundamental to writing correct, efficient C++ code."
  },
  {
    title: "How Templates works",
    definition: "Templates works by compile-time code generation. The compiler creates a new version of the template for each unique set of template arguments (instantiation).",
    explanation: `Under the hood, templates in C++ involves specific compile-time and runtime mechanisms. The compiler enforces rules that ensure correctness.
...
Understanding how templates works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `// Templates - How it works internally
#include <iostream>
using namespace std;
...
public:
    ...
    summary: "Templates works through compile-time code generation. The compiler creates a new version of the template for each unique set of template arguments (instantiation).. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Templates syntax & usage",
    definition: "C++ templates syntax includes template<typename T> for type parameters, template specialization, variadic templates, SFINAE, concepts (C++20), and constexpr if.",
    explanation: `C++ provides clear syntax for templates with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern C++ continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Templates - Syntax patterns
#include <iostream>
#include <memory>
#include <vector>
#include <algorithm>
...
void lambdaDemo() {
    ...
    summary: "C++ syntax for templates is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Templates",
    definition: "In real applications, templates enable generic containers, type-safe callbacks, compile-time computation, and policy-based design in libraries and frameworks.",
    explanation: `Real-world C++ applications use templates for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source C++ projects provide excellent examples of templates in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Templates - Practical game example
#include <iostream>
#include <vector>
#include <memory>
#include <algorithm>
...
int main() {
    ...
    summary: "Real applications demonstrate templates in game systems, data processing, and service design. The patterns are universal across C++ projects."
  },
  {
    title: "Templates best practices",
    definition: "Best practices for templates include keeping templates in headers, providing clear error messages with concepts, minimizing template instantiations, and using CRTP for static polymorphism.",
    explanation: `Professional C++ code follows established conventions for templates that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when templates is used correctly as well-structured code is inherently more testable.
...
public:
    ...
    summary: "Best practices ensure code quality: keeping templates in headers, providing clear error messages with concepts, minimizing template instantiations, and using CRTP for static polymorphism.. Following conventions makes code maintainable and professional."
  }
];
