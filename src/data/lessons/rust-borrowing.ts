// Pre-written lessons for Rust Module: Borrowing

export const rustBorrowingLessons = [
  {
    title: "What is Borrowing?",
    definition: "Borrowing in Rust allows temporary access to values without taking ownership through references (&T for shared/immutable and &mut T for exclusive/mutable). The borrow checker enforces these rules at compile time.",
    explanation: `Borrowing is a core concept in Rust that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Rust's approach to borrowing is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering borrowing, you unlock powerful patterns and idioms that are standard in professional Rust development.`,
    code: `// Borrowing in Rust - Basics
fn main() {
    println!("Borrowing in Rust");
...
fn change_name(s: &mut String) {
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use borrowing in Rust.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Borrowing in Rust provides allows temporary access to values without taking ownership through references (&T for shared/immutable and &mut T for exclusive/mutable). The borrow checker enforces these rules at compile time.. It's fundamental to writing correct, efficient Rust code."
  },
  {
    title: "How Borrowing works",
    definition: "Borrowing works by the compiler verifying that references are always valid (no dangling pointers), no mutable reference coexists with other references, and references don’t outlive the data they point to.",
    explanation: `Under the hood, borrowing in Rust involves specific compile-time and runtime mechanisms. The borrow checker enforces rules that ensure correctness.
...
Understanding how borrowing works internally helps you predict behavior, debug issues, and write more efficient code.
...
fn main() {
    ...
    summary: "Borrowing works through the compiler verifying that references are always valid (no dangling pointers), no mutable reference coexists with other references, and references don’t outlive the data they point to.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Borrowing syntax & usage",
    definition: "Rust borrowing syntax includes &value for immutable borrow, &mut value for mutable borrow, borrowing rules (multiple & OR one &mut), and NLL (Non-Lexical Lifetimes) for more flexible scoping.",
    explanation: `Rust provides clear syntax for borrowing with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Rust continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Borrowing - Syntax patterns
use std::fmt;
...
fn handle_event(event: &GameEvent) {
    ...
    summary: "Rust syntax for borrowing is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Borrowing",
    definition: "In real applications, borrowing enables efficient function parameters without copying, iterator patterns, builder APIs, and safe concurrent access patterns.",
    explanation: `Real-world Rust applications use borrowing for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Rust projects provide excellent examples of borrowing in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Borrowing - Practical example: Game inventory
...
fn main() {
    ...
    summary: "Real applications demonstrate borrowing in game systems, data processing, and service design. The patterns are universal across Rust projects."
  },
  {
    title: "Borrowing best practices",
    definition: "Best practices for borrowing include preferring immutable borrows by default, keeping mutable borrow scopes short, using method receivers (&self, &mut self) appropriately, and understanding how the borrow checker sees your code.",
    explanation: `Professional Rust code follows established conventions for borrowing that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when borrowing is used correctly as well-structured code is inherently more testable.
...
struct Position { x: i32, y: i32 }`,
    ...
    summary: "Best practices ensure code quality: preferring immutable borrows by default, keeping mutable borrow scopes short, using method receivers (&self, &mut self) appropriately, and understanding how the borrow checker sees your code.. Following conventions makes code maintainable and professional."
  }
];
