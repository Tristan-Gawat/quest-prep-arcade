// Pre-written lessons for C++ Module: Memory Management

export const cppMemoryLessons = [
  {
    title: "What is Memory Management?",
    definition: "Memory Management in C++ involves manual control over allocation (new) and deallocation (delete) of heap memory, with stack memory managed automatically through scope-based lifetime.",
    explanation: `Memory Management is a core concept in C++ that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
C++'s approach to memory management is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering memory management, you unlock powerful patterns and idioms that are standard in professional C++ development.`,
    code: `// Memory Management - Basic usage in C++
#include <iostream>
#include <memory>
#include <vector>
using namespace std;
...
int main() {
    ...
    cout << "Memory Management in C++" << endl;
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use memory management in C++.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Memory Management in C++ provides involves manual control over allocation (new) and deallocation (delete) of heap memory, with stack memory managed automatically through scope-based lifetime.. It's fundamental to writing correct, efficient C++ code."
  },
  {
    title: "How Memory Management works",
    definition: "Memory Management works by separating memory into stack (automatic, fast, limited) and heap (manual, flexible, requires explicit deallocation). Memory leaks occur when heap memory is never freed.",
    explanation: `Under the hood, memory management in C++ involves specific compile-time and runtime mechanisms. The compiler enforces rules that ensure correctness.
...
Understanding how memory management works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `// Memory Management - How it works internally
#include <iostream>
using namespace std;
...
public:
    ...
    summary: "Memory Management works through separating memory into stack (automatic, fast, limited) and heap (manual, flexible, requires explicit deallocation). Memory leaks occur when heap memory is never freed.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Memory Management syntax & usage",
    definition: "C++ memory management syntax includes new/delete for single objects, new[]/delete[] for arrays, placement new, and memory functions like malloc/free from C compatibility.",
    explanation: `C++ provides clear syntax for memory management with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern C++ continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Memory Management - Syntax patterns
#include <iostream>
#include <memory>
#include <vector>
#include <algorithm>
...
void lambdaDemo() {
    ...
    summary: "C++ syntax for memory management is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Memory Management",
    definition: "In real applications, memory management is critical for games (pooling), embedded systems (constrained resources), and high-performance applications (avoiding GC pauses).",
    explanation: `Real-world C++ applications use memory management for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source C++ projects provide excellent examples of memory management in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Memory Management - Practical game example
#include <iostream>
#include <vector>
#include <memory>
#include <algorithm>
...
int main() {
    ...
    summary: "Real applications demonstrate memory management in game systems, data processing, and service design. The patterns are universal across C++ projects."
  },
  {
    title: "Memory Management best practices",
    definition: "Best practices for memory management include preferring stack allocation, using RAII, never mixing new/delete with new[]/delete[], tracking ownership clearly, and using memory sanitizers.",
    explanation: `Professional C++ code follows established conventions for memory management that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when memory management is used correctly as well-structured code is inherently more testable.
...
public:
    ...
    summary: "Best practices ensure code quality: preferring stack allocation, using RAII, never mixing new/delete with new[]/delete[], tracking ownership clearly, and using memory sanitizers.. Following conventions makes code maintainable and professional."
  }
];
