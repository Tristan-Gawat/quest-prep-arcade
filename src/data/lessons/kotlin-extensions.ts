// Pre-written lessons for Kotlin Module: Extensions

export const kotlinExtensionsLessons = [
  {
    title: "What is Extensions?",
    definition: "Extensions in Kotlin add new functions and properties to existing classes without modifying their source code or using inheritance. They’re resolved statically at compile time.",
    explanation: `Extensions is a core concept in Kotlin that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Kotlin's approach to extensions is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering extensions, you unlock powerful patterns and idioms that are standard in professional Kotlin development.`,
    code: `// Extensions in Kotlin - Basics
fun main() {
    println("Extensions in Kotlin")
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use extensions in Kotlin.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Extensions in Kotlin provides add new functions and properties to existing classes without modifying their source code or using inheritance. They’re resolved statically at compile time.. It's fundamental to writing correct, efficient Kotlin code."
  },
  {
    title: "How Extensions works",
    definition: "Extensions works by the compiler generating static methods that take the receiver as the first parameter. Extensions don’t actually modify the class — they’re syntactic sugar for utility functions.",
    explanation: `Under the hood, extensions in Kotlin involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how extensions works internally helps you predict behavior, debug issues, and write more efficient code.
...
val hp = 150.clamp(0, 100) // 100`,
    ...
    summary: "Extensions works through the compiler generating static methods that take the receiver as the first parameter. Extensions don’t actually modify the class — they’re syntactic sugar for utility functions.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Extensions syntax & usage",
    definition: "Kotlin extensions syntax includes fun Type.name() for functions, val Type.name for properties, extension on nullable types (Type?.fun), and generic extensions.",
    explanation: `Kotlin provides clear syntax for extensions with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Kotlin continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Extensions - Syntax patterns
...
value class Gold(val amount: Int) {
    ...
    summary: "Kotlin syntax for extensions is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Extensions",
    definition: "In real applications, extensions adds utility methods to String, List, and framework classes; creates DSLs; provides Android view extensions; and improves API readability.",
    explanation: `Real-world Kotlin applications use extensions for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Kotlin projects provide excellent examples of extensions in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Extensions - Practical game example
...
fun CoroutineScope.gameLoop() = launch {
    ...
    summary: "Real applications demonstrate extensions in game systems, data processing, and service design. The patterns are universal across Kotlin projects."
  },
  {
    title: "Extensions best practices",
    definition: "Best practices for extensions include not overusing extensions (they can make code hard to navigate), keeping extensions in logical files, using extensions for utilities not core behavior, and preferring member functions for essential behavior.",
    explanation: `Professional Kotlin code follows established conventions for extensions that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when extensions is used correctly as well-structured code is inherently more testable.
...
Performance and safety are balanced through careful application of these principles. Knowing when to optimize and when readability matters more is a key skill.`,
    code: `// Extensions - Best practices
...
inline fun <T> measureTime(block: () -> T): Pair<T, Long> {
    ...
    summary: "Best practices ensure code quality: not overusing extensions (they can make code hard to navigate), keeping extensions in logical files, using extensions for utilities not core behavior, and preferring member functions for essential behavior.. Following conventions makes code maintainable and professional."
  }
];
