// Pre-written lessons for Go Module: Structs

export const goStructsLessons = [
  {
    title: "What is Structs?",
    definition: "Structs in Go are composite types that group named fields together. They are value types (copied on assignment) and form the basis of Go’s type system alongside interfaces.",
    explanation: `Structs is a core concept in Go that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Go's approach to structs is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering structs, you unlock powerful patterns and idioms that are standard in professional Go development.`,
    code: `// Structs in Go - Basics
package main
...
func main() {
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use structs in Go.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Structs in Go provides are composite types that group named fields together. They are value types (copied on assignment) and form the basis of Go’s type system alongside interfaces.. It's fundamental to writing correct, efficient Go code."
  },
  {
    title: "How Structs works",
    definition: "Structs works by allocating contiguous memory for all fields. Field order affects padding/alignment. Methods with receivers attach behavior to structs without classes.",
    explanation: `Under the hood, structs in Go involves specific compile-time and runtime mechanisms. The runtime scheduler enforces rules that ensure correctness.
...
func main() {
    ...
    summary: "Structs works through allocating contiguous memory for all fields. Field order affects padding/alignment. Methods with receivers attach behavior to structs without classes.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Structs syntax & usage",
    definition: "Go structs syntax includes type Name struct { fields }, struct literals, embedded structs (composition), pointer vs value receivers, tags for metadata, and anonymous structs.",
    explanation: `Go provides clear syntax for structs with several variations depending on your needs. The standard library builds extensively on these foundations.
...
func main() {
    ...
    summary: "Go syntax for structs is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Structs",
    definition: "In real applications, structs models data entities (users, game objects), configuration, API request/response types, and database rows. Embedding provides composition-based code reuse.",
    explanation: `Real-world Go applications use structs for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Go projects provide excellent examples of structs in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Structs - Practical game server
package main
...
func (s *GameServer) Run() {
    ...
    summary: "Real applications demonstrate structs in game systems, data processing, and service design. The patterns are universal across Go projects."
  },
  {
    title: "Structs best practices",
    definition: "Best practices for structs include using pointer receivers for mutating methods, embedding for composition over inheritance, keeping structs focused, adding JSON tags for serialization, and preferring named fields in literals.",
    explanation: `Professional Go code follows established conventions for structs that emerge from years of community experience and real-world usage.
...
func processFile(path string) error {
    ...
    summary: "Best practices ensure code quality: using pointer receivers for mutating methods, embedding for composition over inheritance, keeping structs focused, adding JSON tags for serialization, and preferring named fields in literals.. Following conventions makes code maintainable and professional."
  }
];
