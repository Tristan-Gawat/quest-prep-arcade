// Pre-written lessons for C++ Module: RAII & Smart Pointers

export const cppRAIILessons = [
  {
    title: "What is RAII & Smart Pointers?",
    definition: "RAII & Smart Pointers in C++ (Resource Acquisition Is Initialization) ties resource lifetime to object scope. Smart pointers (unique_ptr, shared_ptr, weak_ptr) automate memory management.",
    explanation: `RAII & Smart Pointers is a core concept in C++ that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
C++'s approach to raii & smart pointers is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering raii & smart pointers, you unlock powerful patterns and idioms that are standard in professional C++ development.`,
    code: `// RAII & Smart Pointers - Basic usage in C++
#include <iostream>
#include <memory>
#include <vector>
using namespace std;
...
int main() {
    ...
    cout << "RAII & Smart Pointers in C++" << endl;
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use raii & smart pointers in C++.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "RAII & Smart Pointers in C++ provides (Resource Acquisition Is Initialization) ties resource lifetime to object scope. Smart pointers (unique_ptr, shared_ptr, weak_ptr) automate memory management.. It's fundamental to writing correct, efficient C++ code."
  },
  {
    title: "How RAII & Smart Pointers works",
    definition: "RAII & Smart Pointers works by acquiring resources in constructors and releasing in destructors. When objects go out of scope, destructors run automatically, ensuring cleanup even during exceptions.",
    explanation: `Under the hood, raii & smart pointers in C++ involves specific compile-time and runtime mechanisms. The compiler enforces rules that ensure correctness.
...
Understanding how raii & smart pointers works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `// RAII & Smart Pointers - How it works internally
#include <iostream>
using namespace std;
...
public:
    ...
    summary: "RAII & Smart Pointers works through acquiring resources in constructors and releasing in destructors. When objects go out of scope, destructors run automatically, ensuring cleanup even during exceptions.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "RAII & Smart Pointers syntax & usage",
    definition: "C++ raii & smart pointers syntax includes unique_ptr<T> for exclusive ownership, shared_ptr<T> for shared ownership, weak_ptr<T> for non-owning references, and make_unique/make_shared factory functions.",
    explanation: `C++ provides clear syntax for raii & smart pointers with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern C++ continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// RAII & Smart Pointers - Syntax patterns
#include <iostream>
#include <memory>
#include <vector>
#include <algorithm>
...
void lambdaDemo() {
    ...
    summary: "C++ syntax for raii & smart pointers is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of RAII & Smart Pointers",
    definition: "In real applications, raii & smart pointers eliminates memory leaks in game engines, manages file handles, database connections, GPU resources, and network sockets safely.",
    explanation: `Real-world C++ applications use raii & smart pointers for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source C++ projects provide excellent examples of raii & smart pointers in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// RAII & Smart Pointers - Practical game example
#include <iostream>
#include <vector>
#include <memory>
#include <algorithm>
...
int main() {
    ...
    summary: "Real applications demonstrate raii & smart pointers in game systems, data processing, and service design. The patterns are universal across C++ projects."
  },
  {
    title: "RAII & Smart Pointers best practices",
    definition: "Best practices for raii & smart pointers include preferring unique_ptr by default, using make_unique/make_shared, avoiding raw new/delete, passing smart pointers by reference to avoid copies, and using weak_ptr to break cycles.",
    explanation: `Professional C++ code follows established conventions for raii & smart pointers that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when raii & smart pointers is used correctly as well-structured code is inherently more testable.
...
Performance and safety are balanced through careful application of these principles. Knowing when to optimize and when readability matters more is a key skill.`,
    code: `// RAII & Smart Pointers - Best practices
#include <memory>
#include <vector>
#include <string>
using namespace std;
...
public:
    ...
    summary: "Best practices ensure code quality: preferring unique_ptr by default, using make_unique/make_shared, avoiding raw new/delete, passing smart pointers by reference to avoid copies, and using weak_ptr to break cycles.. Following conventions makes code maintainable and professional."
  }
];
