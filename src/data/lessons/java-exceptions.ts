// Pre-written lessons for Java Module: Exception Handling

export const javaExceptionsLessons = [
  {
    title: "What is Exception Handling?",
    definition: "Exception handling in Java uses try-catch-finally blocks to handle runtime errors gracefully. Java has checked exceptions (must handle) and unchecked exceptions (runtime errors like NullPointerException).",
    explanation: `Exception Handling is a fundamental concept in Java programming that enables writing more organized, efficient, and maintainable code. Understanding exception handling deeply is essential for any Java developer.

At its core, exception handling provides abstractions that reduce code duplication and improve type safety. Rather than writing similar code for different scenarios, you define the pattern once and reuse it.

Java's implementation of exception handling is designed to be both powerful and practical. It integrates seamlessly with the language's existing type system and standard library.

Mastering exception handling opens up advanced design patterns and architectural approaches that are standard in professional Java development.`,
    code: `// Exception Handling in Java - Basic usage\npublic class Example {\n    // Core concept demonstration\n    public static void main(String[] args) {\n        System.out.println("Exception Handling basics");\n\n        // Fundamental operations\n        var data = new ArrayList<String>();\n        data.add("Sword");\n        data.add("Shield");\n        data.add("Potion");\n\n        // Process with modern Java\n        data.stream()\n            .filter(item -> item.length() > 5)\n            .forEach(System.out::println);\n\n        // Type-safe operations\n        for (String item : data) {\n            System.out.printf("Item: %s (%d chars)%n",\n                item, item.length());\n        }\n    }\n}`,
    breakdown: `\u2022 Core concepts demonstrated with practical examples showing how exception handling works in everyday Java code.\n\n\u2022 Type safety ensures compile-time error detection rather than runtime crashes.\n\n\u2022 Modern Java features like var, streams, and enhanced for-each make code concise and readable.\n\n\u2022 Standard library provides ready-made implementations for common patterns.`,
    summary: "Exception Handling in Java provides type-safe, efficient abstractions for common programming patterns. The standard library offers ready-made implementations. Modern Java syntax makes usage concise and readable."
  },
  {
    title: "How Exception Handling works",
    definition: "When an exception is thrown, the JVM unwinds the call stack looking for a matching catch block. If none is found, the thread terminates. The finally block always executes regardless of exceptions.",
    explanation: `Under the hood, exception handling in Java involves compile-time checks and runtime mechanisms that ensure correctness and performance. The JVM optimizes common patterns for maximum efficiency.
...
Memory management and object lifecycle are handled automatically by the garbage collector, but understanding how exception handling interacts with memory helps you write more efficient code.
...
Performance characteristics vary by implementation. Understanding the tradeoffs helps you choose the right approach for your specific use case.`,
    ...
    summary: "Java handles exception handling through compile-time checks and JVM optimization. Understanding internal mechanics helps choose the right implementation for performance-critical code."
  },
  {
    title: "Exception Handling syntax & usage",
    definition: "Exception syntax includes try/catch/finally, throw, throws declarations, custom exception classes, try-with-resources (auto-closing), and multi-catch (catch Type1 | Type2).",
    explanation: `Java provides rich syntax for exception handling with multiple approaches depending on the complexity of your needs. The standard library provides ready-made implementations for common cases.
...
Modern Java (11+) continues to add syntactic improvements that make exception handling code more concise while maintaining backwards compatibility.`,
  ...
  },
  {
    ...
    definition: "Exceptions handle I/O failures, network timeouts, invalid user input, database errors, and business rule violations. Custom exceptions communicate domain-specific error conditions clearly.",
    explanation: `Real-world Java applications use exception handling extensively for data processing, API design, game mechanics, and enterprise systems. The patterns you learn here appear in every significant Java codebase.

Game development uses exception handling for entity management, event systems, and data pipelines. Web applications use it for request handling, data transformation, and service composition.
...
Open-source Java projects demonstrate real-world usage patterns that you can study and apply to your own code.`,
    ...
    summary: "Real applications use exception handling for entity management, data pipelines, statistics aggregation, and game systems. Streams provide declarative alternatives to manual loops."
  },
  {
    title: "Exception Handling best practices",
    definition: "Best practices: catch specific exceptions not generic Exception, use try-with-resources for all Closeable resources, don't use exceptions for flow control, include context in error messages, prefer unchecked for programming errors.",
    explanation: `Professional Java development follows established conventions for exception handling that make code readable, maintainable, and robust. These conventions emerge from decades of community experience.

Code review standards in Java projects emphasize proper use of exception handling — it's one of the first things reviewers check. Following best practices signals professional competence.

Testing is easier when exception handling is used correctly. Well-structured code using these patterns is inherently more testable due to loose coupling and clear interfaces.
