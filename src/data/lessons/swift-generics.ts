// Pre-written lessons for Swift Module: Generics

export const swiftGenericsLessons = [
  {
    title: "What is Generics?",
    definition: "Generics in Swift enable writing flexible, reusable functions and types that work with any type meeting specified constraints. They’re the foundation of Swift’s standard library (Array<T>, Optional<T>, Result<T,E>).",
    explanation: `Generics is a core concept in Swift that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Swift's approach to generics is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering generics, you unlock powerful patterns and idioms that are standard in professional Swift development.`,
    code: `// Generics in Swift - Basics
import Foundation
...
print("\(hero.name): \(hero.hp)/\(hero.maxHp)")`,
    breakdown: `\u2022 The basic declaration shows how to define and use generics in Swift.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Generics in Swift provides enable writing flexible, reusable functions and types that work with any type meeting specified constraints. They’re the foundation of Swift’s standard library (Array<T>, Optional<T>, Result<T,E>).. It's fundamental to writing correct, efficient Swift code."
  },
  {
    title: "How Generics works",
    definition: "Generics works by the compiler generating specialized code for each concrete type used (monomorphization for performance). Type constraints ensure only valid types are accepted.",
    explanation: `Under the hood, generics in Swift involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how generics works internally helps you predict behavior, debug issues, and write more efficient code.
...
enum GameState {
    ...
    summary: "Generics works through the compiler generating specialized code for each concrete type used (monomorphization for performance). Type constraints ensure only valid types are accepted.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Generics syntax & usage",
    definition: "Swift generics syntax includes func name<T>(param: T), struct Name<T>, protocol with associatedtype, where clauses for constraints, and some/any keywords for opaque/existential types.",
    explanation: `Swift provides clear syntax for generics with several variations depending on your needs. The standard library builds extensively on these foundations.
...
case .failure(let error):
    ...
    summary: "Swift syntax for generics is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Generics",
    definition: "In real applications, generics enables type-safe collections, generic algorithms, protocol-oriented programming, and framework APIs that work with user-defined types.",
    explanation: `Real-world Swift applications use generics for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Swift projects provide excellent examples of generics in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Generics - Practical game system
...
func handle(_ event: GameEvent) {
    ...
    summary: "Real applications demonstrate generics in game systems, data processing, and service design. The patterns are universal across Swift projects."
  },
  {
    title: "Generics best practices",
    definition: "Best practices for generics include using constraints (T: Protocol) to enable functionality, preferring some over any for performance, keeping generic signatures simple, and using type aliases for complex generic types.",
    explanation: `Professional Swift code follows established conventions for generics that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when generics is used correctly as well-structured code is inherently more testable.
...
// DO: Use if-let or guard-let instead`,
    ...
    summary: "Best practices ensure code quality: using constraints (T: Protocol) to enable functionality, preferring some over any for performance, keeping generic signatures simple, and using type aliases for complex generic types.. Following conventions makes code maintainable and professional."
  }
];
