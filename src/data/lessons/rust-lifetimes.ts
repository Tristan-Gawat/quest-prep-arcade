// Pre-written lessons for Rust Module: Lifetimes

export const rustLifetimesLessons = [
  {
    title: "What is Lifetimes?",
    definition: "Lifetimes in Rust are annotations (’a) that tell the compiler how long references are valid. They prevent dangling references by ensuring borrowed data lives at least as long as the reference.",
    explanation: `Lifetimes is a core concept in Rust that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Rust's approach to lifetimes is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering lifetimes, you unlock powerful patterns and idioms that are standard in professional Rust development.`,
    code: `// Lifetimes in Rust - Basics
fn main() {
...
fn change_name(s: &mut String) {
    ...
    breakdown: `\u2022 The basic declaration shows how to define and use lifetimes in Rust.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Lifetimes in Rust provides are annotations (’a) that tell the compiler how long references are valid. They prevent dangling references by ensuring borrowed data lives at least as long as the reference.. It's fundamental to writing correct, efficient Rust code."
  },
  {
    title: "How Lifetimes works",
    definition: "Lifetimes works by the compiler using lifetime annotations to verify that all references are valid for their entire usage. Lifetimes are checked at compile time with zero runtime cost.",
    explanation: `Under the hood, lifetimes in Rust involves specific compile-time and runtime mechanisms. The borrow checker enforces rules that ensure correctness.
...
Understanding how lifetimes works internally helps you predict behavior, debug issues, and write more efficient code.
...
fn main() {
    ...
    summary: "Lifetimes works through the compiler using lifetime annotations to verify that all references are valid for their entire usage. Lifetimes are checked at compile time with zero runtime cost.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Lifetimes syntax & usage",
    definition: "Rust lifetimes syntax includes ’a annotations on references (&’a T), lifetime elision rules (compiler infers in common cases), ’static for program-duration references, and lifetime bounds on generics.",
    explanation: `Rust provides clear syntax for lifetimes with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Rust continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `// Lifetimes - Syntax patterns
use std::fmt;
...
fn handle_event(event: &GameEvent) {
    ...
    summary: "Rust syntax for lifetimes is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Lifetimes",
    definition: "In real applications, lifetimes ensures memory safety in data structures holding references, iterator implementations, parsers that reference input data, and zero-copy APIs.",
    explanation: `Real-world Rust applications use lifetimes for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Rust projects provide excellent examples of lifetimes in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `// Lifetimes - Practical example: Game inventory
...
fn main() {
    ...
    summary: "Real applications demonstrate lifetimes in game systems, data processing, and service design. The patterns are universal across Rust projects."
  },
  {
    title: "Lifetimes best practices",
    definition: "Best practices for lifetimes include relying on lifetime elision when possible, using owned data (String, Vec) instead of references for simplicity, understanding that lifetimes describe existing relationships (don’t create them), and using ’static only when truly needed.",
    explanation: `Professional Rust code follows established conventions for lifetimes that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when lifetimes is used correctly as well-structured code is inherently more testable.
...
struct Position { x: i32, y: i32 }`,
    ...
    summary: "Best practices ensure code quality: relying on lifetime elision when possible, using owned data (String, Vec) instead of references for simplicity, understanding that lifetimes describe existing relationships (don’t create them), and using ’static only when truly needed.. Following conventions makes code maintainable and professional."
  }
];
