// Pre-written lessons for Ruby Module: Blocks & Procs

export const rubyBlocksLessons = [
  {
    title: "What is Blocks & Procs?",
    definition: "Blocks & Procs in Ruby are anonymous chunks of code passed to methods, enabling powerful iteration patterns, callbacks, and DSLs. Procs and lambdas are objects that store blocks for later execution.",
    explanation: `Blocks & Procs is a core concept in Ruby that every developer needs to master. It provides the foundation for writing efficient, safe, and maintainable code.
...
Ruby's approach to blocks & procs is unique among programming languages, offering specific guarantees and trade-offs that shape how you design your programs.

By mastering blocks & procs, you unlock powerful patterns and idioms that are standard in professional Ruby development.`,
    code: `# Blocks & Procs in Ruby - Basics
...
hero.take_damage(40)
puts hero`,
    breakdown: `\u2022 The basic declaration shows how to define and use blocks & procs in Ruby.\n\n\u2022 Type safety ensures the compiler catches errors before runtime.\n\n\u2022 Standard library integration makes common operations concise.\n\n\u2022 Comments explain each line's purpose for learners.\n\n\u2022 The example demonstrates the most common usage pattern.`,
    summary: "Blocks & Procs in Ruby provides are anonymous chunks of code passed to methods, enabling powerful iteration patterns, callbacks, and DSLs. Procs and lambdas are objects that store blocks for later execution.. It's fundamental to writing correct, efficient Ruby code."
  },
  {
    title: "How Blocks & Procs works",
    definition: "Blocks & Procs works by passing code as arguments to methods. The method calls yield to execute the block. Procs store blocks as objects. Lambdas are strict Procs that check argument count.",
    explanation: `Under the hood, blocks & procs in Ruby involves specific compile-time and runtime mechanisms. The interpreter enforces rules that ensure correctness.
...
Understanding how blocks & procs works internally helps you predict behavior, debug issues, and write more efficient code.
...
This knowledge separates intermediate developers from advanced ones and is the difference between using a feature and truly understanding it.`,
    code: `# Blocks & Procs - How it works
...
puts names.map(&:capitalize) # ["Alice", "Bob", "Charlie"]`,
    ...
    summary: "Blocks & Procs works through passing code as arguments to methods. The method calls yield to execute the block. Procs store blocks as objects. Lambdas are strict Procs that check argument count.. Understanding internals helps you write better code and debug effectively."
  },
  {
    title: "Blocks & Procs syntax & usage",
    definition: "Ruby blocks & procs syntax includes { |args| code } or do |args| code end for blocks, Proc.new/proc{} for procs, ->(args){} or lambda{} for lambdas, yield to call blocks, &block to capture.",
    explanation: `Ruby provides clear syntax for blocks & procs with several variations depending on your needs. The standard library builds extensively on these foundations.
...
Modern Ruby continues to evolve, adding syntactic improvements while maintaining backwards compatibility with existing code.`,
    code: `# Blocks & Procs - Syntax patterns
...
class DynamicFinder
  def method_missing(method, *args)
    ...
    summary: "Ruby syntax for blocks & procs is expressive and type-safe. Multiple forms serve different needs from simple to complex use cases."
  },
  {
    title: "Practical examples of Blocks & Procs",
    definition: "In real applications, blocks & procs enables Rails callbacks, custom iterators, DSLs (domain-specific languages), resource management patterns, and functional programming in Ruby.",
    explanation: `Real-world Ruby applications use blocks & procs for data processing, system design, and performance-critical code paths. These patterns appear in production codebases everywhere.

Game development, web services, and system programming all leverage these concepts extensively.

Open-source Ruby projects provide excellent examples of blocks & procs in action. Studying them accelerates your learning.
...
The patterns you learn here transfer to related problems. Once you understand the principles, applying them to new situations becomes natural.`,
    code: `# Blocks & Procs - Practical game example
...
puts "Most valuable: #{inv.max_by { |i| i[:value] }[:name]}"`,
    ...
    summary: "Real applications demonstrate blocks & procs in game systems, data processing, and service design. The patterns are universal across Ruby projects."
  },
  {
    title: "Blocks & Procs best practices",
    definition: "Best practices for blocks & procs include preferring blocks for simple iteration, using lambdas when argument checking matters, using &method(:name) for method references, and keeping blocks short and readable.",
    explanation: `Professional Ruby code follows established conventions for blocks & procs that emerge from years of community experience and real-world usage.

Code review standards emphasize proper usage of these patterns. Following best practices signals professional competence.

Testing is easier when blocks & procs is used correctly as well-structured code is inherently more testable.
...
Performance and safety are balanced through careful application of these principles. Knowing when to optimize and when readability matters more is a key skill.`,
    code: `# Blocks & Procs - Best practices
...
# = for setters: name= hp=`,
    ...
    summary: "Best practices ensure code quality: preferring blocks for simple iteration, using lambdas when argument checking matters, using &method(:name) for method references, and keeping blocks short and readable.. Following conventions makes code maintainable and professional."
  }
];
